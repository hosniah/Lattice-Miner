...
public class ConceptLattice {
	...
	/**
	 * Trouve tous les noeuds rejoignable à partir du noeud spécifié, en incluant celui-ci
	 * @param node Le ConceptNode à partir duquel les noeuds doivent être cherchés
	 * @return Vector La liste des ConceptNode accessibles à partir du noeud spécifié
	 */
	...
	private Vector<FormalConcept> findConcepts(FormalConcept node, boolean bottomFirst) {
		...
		if (!bottomFirst) {
			/* Ajout des enfants qui n'ont pas encore été ajoutés */
			...
			for (int i = 0; i < children.size(); i++) {
				...
				/* Ajout des noeuds visités à partir des enfants et qui n'ont pas encore été ajoutés */
			...
			/* Ajout des parents qui n'ont pas encore été ajoutés */
			...
			for (int i = 0; i < parents.size(); i++) {
				...
				/* Ajout des noeuds visités à partir des parents et qui n'ont pas encore été ajoutés */
	...
	/**
	 * Permet d'obtenir le concept qui possède l'intention spécifiée. Retourne null si aucun concept
	 * n'est trouvé.
	 * @param intent Le BasicSet contenant l'intent recherché
	 * @return Le FormalConcept qui possède l'intention spécifiée
	 */
	public FormalConcept getConceptWithIntent(BasicSet intent) {
		...
		/* Le concept recherche n'est pas le suprémum */
		else {
			/* Recherche du noeud */
			...
			/* Les attributs ont tous été trouvés */
			if (remainder.size() == 0)
