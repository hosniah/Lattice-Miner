...
/**
 * Fenêtre affichant un treillis (imbriqué ou non) avec sa structure arborescente et son panneau de
 * contrôle
...
public class LatticeViewer extends Viewer {
	...
	LatticePanel viewer; //Le panneau chargé d'afficher le treillis imbriqué
	...
	Tree treePanel; //Le panneau chargé d'afficher la structure arborescente du treillis
	...
	/**
	 * Constructeur
	 * @param gl Le NestedGraphLattice qui doit être affiché
	 */
	public LatticeViewer(GraphicalLattice gl) {
		...
		// Par defaut c'est le User Options qui est selectionné et on bloque l'historique pour ne pas voir l'opération
		viewer.lockHistory();
	...
	private JPanel buildPanel(GraphicalLattice gl) {
		...
		/* Création du panneau affichant le treillis imbriqué */
		...
		/* Création du panneau pour la structure arborescente du treillis imbriqué */
		...
		/* Le panneau est sensible aux événements dans la structure arborescente */
		treePanel.getTree().addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent e) {
				/*
				 * Changement de sélection dans l'arbre : Le noeud du treillis graphique
				 * correspondant doit être sélectionné et visible, donc le zoom est ajusté pour que
				 * le treillis du noeud soit au centre de l'écran
				 */
				if (e.getPropertyName().equals("leadSelectionPath")) { //$NON-NLS-1$
					GraphicalConcept selectedNode = treePanel.getSelectedNode();
					/* Sélection du noeud racine de l'arbre */
				...
				/*
				 * Changement de modèle pour l'arbre (exemple : projection) : Comme la projection
				 * modifie aussi le noeud sélectionné, il est important d'ajuster le zoom pour
		...
		// "Search/Approximate" différents si imbriqué ou non
		...
		/* Ajout des panneau dans la fenêtre */
	...
	/**
	 * Construit une fenÃªtre qui permettra de modifier la structure des treillis
	 */
	private void openStructureViewer() throws AlreadyExistsException, InvalidTypeException {
		/* Construction d'un vecteur contenant chacun des treillis du treillis imbriquï¿½ */
		...
		/* Construction d'un vecteur contenant les structures de chacun des treillis imbriquï¿½ */
	...
	/**
	 * Duplique la fenetre pour {@link GraphicalLattice} donné
	 * @param gl le GraphicalLattice a afficher dans la nouvelle fenêtre
	 */
...
//		}
	...
	private void saveCurrentLatticeAs() {
		...
		if (!glCurrent.isNested()) {
			...
			// Recupère le treillis non imnriqué
...
//		}
	...
	/**
	 * Classe gérant le menu du {@link LatticeViewer}
	...
	private class FrameMenu {
		...
		public FrameMenu() {
			...
			// Desactive varySizeItem si pas de treillis imbriqué
		...
		private void buildMenuBar() {
			...
			duplicateItem.setAccelerator(KeyStroke.getKeyStroke("ctrl D")); //$NON-NLS-1$
			fileMenu.add(duplicateItem);
			
			fileMenu.addSeparator();
			
			// On ne peut sauvegarder qu'un treillis non imbriqué
			if (!glCurrent.isNested()) {
				...
				saveLattice.setAccelerator(KeyStroke.getKeyStroke("ctrl S")); //$NON-NLS-1$
			...
			exportLattice.setAccelerator(KeyStroke.getKeyStroke("ctrl E")); //$NON-NLS-1$
			fileMenu.add(exportLattice);
			
			fileMenu.addSeparator();
			
			quitFrame = new JMenuItem(GUIMessages.getString("GUI.quit")); //$NON-NLS-1$
			quitFrame.addActionListener(listener);
			quitFrame.setMnemonic(KeyEvent.VK_Q);
			quitFrame.setAccelerator(KeyStroke.getKeyStroke("ctrl Q")); //$NON-NLS-1$
			...
			undoOperation.setAccelerator(KeyStroke.getKeyStroke("ctrl Z")); //$NON-NLS-1$
			...
			redoOperation.setAccelerator(KeyStroke.getKeyStroke("ctrl Y")); //$NON-NLS-1$
			...
			zoomSelectItem.setAccelerator(KeyStroke.getKeyStroke("ctrl shift S")); //$NON-NLS-1$
			...
			zoomLatticeItem.setAccelerator(KeyStroke.getKeyStroke("ctrl shift A")); //$NON-NLS-1$
			...
			hideOutFocusLabelItem.setAccelerator(KeyStroke.getKeyStroke("ctrl shift H")); //$NON-NLS-1$
			...
			showBottomItem.setAccelerator(KeyStroke.getKeyStroke("ctrl I")); //$NON-NLS-1$
			...
			showLatticeMapItem.setAccelerator(KeyStroke.getKeyStroke("ctrl H")); //$NON-NLS-1$
			...
			latticeStructureItem.setAccelerator(KeyStroke.getKeyStroke("ctrl M")); //$NON-NLS-1$
			...
			rulesItem.setAccelerator(KeyStroke.getKeyStroke("ctrl R")); //$NON-NLS-1$
